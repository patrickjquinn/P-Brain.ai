...
router.get(
    ...
    wrap(async function(req, res) {
        ...
        if (req.user.is_admin) {
            const users = await global.db.getUsers();
            res.json(users);
        } else {
            res.json([req.user]);
        }
...
router.get(
    ...
    wrap(async function(req, res) {
        if (req.user.is_admin || req.params.user == req.user.username) {
            const target_user = await global.db.getUser({ username: req.params.user });
            if (target_user) {
                if (req.params.delete) {
                    // This branch is for deleting an existing user.
                    await global.db.deleteUser(target_user);
                    ...
                    if (req.query.username || req.query.password || req.query.is_admin) {
                        ...
                        if (req.query.password) {
                            target_user.password = await global.auth.encryptPassword(
                                req.query.password,
                            );
                        }
                        if (
                            ...
                            target_user.is_admin =
                                req.query.is_admin === true || req.query.is_admin == 'true';
                        }
                        console.log('Updating user.');
                        try {
                            console.log(target_user);
                            await global.db.saveUser(target_user);
                            res.json(await global.db.getUser({ username: target_user.username }));
                        } catch (err) {
                            res.status(503).json({
                ...
                if (req.user.is_admin && req.query.password && req.query.is_admin) {
                    ...
                    const target_user = {
                        ...
                        password: await global.auth.encryptPassword(req.query.password),
                    ...
                    res.status(404).json({
                        error: "Not enough parameters supplied or you're not an admin.",
                    });
                }
            }
        } else {
