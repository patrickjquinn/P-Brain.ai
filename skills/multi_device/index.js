...
function hardRule(query) {
    return (
        query.startsWith('call this device') ||
        query.startsWith('this device is called') ||
        query.startsWith('on my') ||
        query.startsWith('on the')
    );
}

async function multi_resp(query, breakdown, user, token) {
    if (query.includes('call this device') || query.includes('this device is called')) {
        // Parse queries for setting the device name.
        let device = query
            .replace('call this device', '')
            .replace('this device is called', '')
            .trim();
        ...
        const tokens = await global.db.getUserTokens(user, { name: device });
        if (tokens.length > 0) {
        ...
        query = query
            ...
            .replace('on the', '')
            .trim();
        const tokens = await global.db.getUserTokens(user);
        ...
        if (found_device) {
            ...
            const data = await global.query(command, user, found_device);
...
const examples = () => [
    'Call this device my laptop.',
    'Call this device the TV.',
    'On my TV play Game Of Thrones',
    'On the laptop tell me the time.',
];
