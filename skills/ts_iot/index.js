...
async function do_iot(device, state) {
    ...
    const iot_url = await global.db.getSkillValue('ts_iot', 'url');
    if (iot_url) {
        const parsed_device = device.split(' ').join('_');
        let data = await request(util.format(iot_url, parsed_device, state));
...
async function iot_resp(query) {
    ...
    if (query.startsWith('turn on the') || query.startsWith('turn off the')) {
        ...
        // Parse the query with the form 'Turn the light on.'.
        if (words.length > 3) {
        ...
        // Parse the form 'Is the light on?'.
        if (words.length > 3) {
            state = 'state';
    ...
    return await do_iot(device, state);
}

const examples = () => ['Turn on the light.', 'Turn the light off.', 'Is the light on?'];
