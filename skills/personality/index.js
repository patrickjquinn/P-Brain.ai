...
async function conditionalRule(query, breakdown, user, device, isResponse) {
    if (isResponse === true || isResponse == null || isResponse == undefined) {
        const responses = await global.db.getResponses(
            user,
            undefined,
            device,
            Date.now() / 1000 - CONVERSATION_TIME_LIMIT,
        );
...
async function getConversation(user, device) {
    return await global.db.getResponses(
        user,
        'personality',
        device,
        Date.now() / 1000 - CONVERSATION_TIME_LIMIT,
...
async function resp(query, breakdown, user, device) {
    if (query.startsWith('how are you')) {
        ...
        if (query.trim() == 'what do you think about') {
            ...
            return {
                text: `I have no opinion on ${query
        ...
        query.startsWith('im feeling') ||
        query.startsWith('i feel') ||
        query.startsWith('im not feeling')
    ) {
        ...
        const conversation = await getConversation(user, device);
