...
const examples = () => [
    ...
    'Set a timer for 40 seconds then tell me the time',
    'In 2 hours make me laugh.',
];
...
function initializeClock(time, command, user, device) {
    ...
    function updateClock() {
        if (Date.now() >= timer.deadline) {
            ...
            if (timer.command) {
                (async function() {
                    const data = await global.query(timer.command, timer.user, timer.device);
...
async function timer_resp(query, breakdown, user, device) {
    ...
    if (query.startsWith('show') || query.startsWith('what')) {
        ...
        if (user_timers.length > 0) {
            timersString = `You have ${user_timers.length == 1 ? 'a ' : ''}timer${
                user_timers.length > 1 ? 's' : ''
            } for: `;
            for (let i = 0; i < user_timers.length; i++) {
                const formatted = formatTime(
                    getTimeRemaining(user_timers[i].deadline.getTime() - Date.now()),
                );
                const command = user_timers[i].command
                    ? ` which will run '${user_timers[i].command}'`
                    : '';
                timersString += `${i > 0 ? ', ' : ''}${
                    i > 0 && i == user_timers.length - 1 ? 'and ' : ''
                }${formatted}${command}`;
            }
        }
